...
-callback websocket_init(cowboy_req:req(),State)
...
takeover(Parent, Ref, Socket, Transport, _Opts, Buffer,
	...
	case erlang:function_exported(Handler, websocket_init, 2) of
...
handler_call(State=#state{handler=Handler,req = Req}, HandlerState,
		ParseState, Callback, Message, NextState) ->
	try case Callback of
		websocket_init -> Handler:websocket_init(Req,HandlerState);
